...
QUERIES = {
    "workflow_billing_data": {
        ...
        "query": """SELECT DISTINCT ON (workflow_billing_db.id) project_db.team_id AS team_id, workflow_billing_db.id, workflow_billing_db.app_name, workflow_billing_db.start_time, workflow_billing_db.exit_time, workflow_billing_db.gpu
...
ORDER BY workflow_billing_db.id DESC, workflow_billing_db.start_time DESC""",
    ...
    "users_per_team": {
        ...
        "query": """SELECT DISTINCT u.email, t.id AS team_id
FROM user_db u
...
ORDER BY t.id""",
    ...
    "new_subscribers": {
        ...
        "query": """SELECT u.email, p.type, t.id AS team_id, s.created_at
FROM team_db t
...
ORDER BY s.created_at DESC""",
        "params": ((1921,1882,1883,1884,1888,1881,1879,1856),),  # list of stripe_billing_info_db.id
    },
    "update_billing_status": {
        "out_path": None,
        "query": """UPDATE workflow_billing_db
...
AND billing_status = 'PENDING'""",
        "params": ((1069,3664,3781,3770,4264,1961,4188,4222,4417), "2026-03-11 12:10:43"),  # (team_ids_tuple, exit_time_cutoff)
    },
}
